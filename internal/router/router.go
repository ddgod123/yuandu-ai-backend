...
func Setup(cfg config.Config, db *gorm.DB, qiniu *storage.QiniuClient, ossClient *oss.Client, ai *service.AIService, compose *service.ComposeService) *gin.Engine {
	...
	api := r.Group("/api")
	{
		...
		api.GET("/upload-rules", h.GetUploadRules)
		...
		api.POST("/integrations/feishu/events", h.FeishuEventCallback)
		api.POST("/gpu-tests/jobs/:id/callback", h.CallbackGPUImageEnhanceJob)
		...
		auth := api.Group("", middleware.Auth(cfg))
		{
			...
			auth.POST("/emojis/:id/like", h.AddEmojiLike)
			auth.DELETE("/emojis/:id/like", h.RemoveEmojiLike)
			auth.GET("/emojis/:id/download", h.GetEmojiDownload)
			auth.GET("/emojis/:id/download-file", h.DownloadEmojiFile)
			auth.GET("/me", h.Me)
			auth.PUT("/me", h.UpdateMe)
			auth.GET("/me/integrations/overview", h.GetMyIntegrationsOverview)
			...
			auth.POST("/integrations/feishu/bind-code/confirm", h.ConfirmFeishuBindCode)
			auth.POST("/integrations/mock/qq/video-jobs", h.CreateMockQQVideoJob)
			auth.POST("/integrations/mock/wecom/video-jobs", h.CreateMockWeComVideoJob)
			...
			auth.POST("/gpu-tests/upload-token", h.GetGPUTestUploadToken)
			auth.POST("/gpu-tests/jobs", h.CreateGPUImageEnhanceJob)
			auth.GET("/gpu-tests/jobs", h.ListGPUImageEnhanceJobs)
			auth.POST("/gpu-tests/jobs/batch-delete", h.BatchDeleteGPUImageEnhanceJobs)
			auth.GET("/gpu-tests/jobs/:id", h.GetGPUImageEnhanceJob)
			auth.DELETE("/gpu-tests/jobs/:id", h.DeleteGPUImageEnhanceJob)
			...
			auth.GET("/me/uploads/collections", h.ListMyUploadCollections)
			auth.GET("/me/uploads/overview", h.GetMyUploadOverview)
			auth.GET("/me/uploads/review/collections", h.ListMyUploadReviewCollections)
			auth.GET("/me/uploads/collections/:id/review-detail", h.GetMyUploadReviewCollectionDetail)
			auth.POST("/me/uploads/collections", h.CreateMyUploadCollection)
			auth.GET("/me/uploads/collections/:id", h.GetMyUploadCollection)
			auth.PUT("/me/uploads/collections/:id", h.UpdateMyUploadCollection)
			auth.DELETE("/me/uploads/collections/:id", h.DeleteMyUploadCollection)
			auth.GET("/me/uploads/collections/:id/emojis", h.ListMyUploadCollectionEmojis)
			auth.GET("/me/uploads/collections/:id/review-logs", h.ListMyUploadCollectionReviewLogs)
			auth.POST("/me/uploads/collections/:id/emojis/upload", h.UploadMyCollectionEmojis)
			auth.POST("/me/uploads/collections/:id/submit-review", h.SubmitMyUploadCollectionReview)
			auth.POST("/me/uploads/collections/:id/publish", h.PublishMyUploadCollection)
			auth.POST("/me/uploads/collections/:id/unpublish", h.UnpublishMyUploadCollection)
			auth.PUT("/me/uploads/emojis/:id", h.UpdateMyUploadEmoji)
			auth.DELETE("/me/uploads/emojis/:id", h.DeleteMyUploadEmoji)
		...
		admin := api.Group("/admin", middleware.Auth(cfg), middleware.RequireAnyRole("super_admin"))
		{
			...
			admin.GET("/ips/binding-metrics", h.GetAdminIPBindingMetrics)
			admin.GET("/ips/:id", h.GetAdminIP)
			admin.GET("/ips/:id/bindings", h.ListAdminIPBindings)
			admin.GET("/ips/:id/bindings/logs", h.ListAdminIPBindingAuditLogs)
			admin.POST("/ips", h.CreateIP)
			admin.PUT("/ips/:id", h.UpdateIP)
			admin.DELETE("/ips/:id", h.DeleteIP)
			admin.POST("/ips/:id/bindings", h.UpsertAdminIPBinding)
			admin.POST("/ips/:id/bindings/batch-import", h.BatchImportAdminIPBindings)
			admin.PUT("/ips/:id/bindings/:binding_id", h.UpdateAdminIPBinding)
			admin.DELETE("/ips/:id/bindings/:binding_id", h.DeleteAdminIPBinding)
			admin.POST("/ips/:id/bindings/reorder", h.ReorderAdminIPBindings)
			...
			admin.GET("/analytics/dashboard", h.GetAdminAnalyticsDashboard)
			admin.POST("/analytics/geo-backfill", h.BackfillAdminAnalyticsGeo)
			admin.GET("/system/worker-health", h.GetAdminWorkerHealth)
			admin.GET("/system/doctor", h.GetAdminSystemDoctor)
			admin.POST("/system/local/gif-zip-to-webp", h.ConvertLocalGIFZipToWebP)
			admin.POST("/system/local/gif-zip-to-webp/jobs", h.StartConvertLocalGIFZipToWebPJob)
			admin.GET("/system/local/gif-zip-to-webp/jobs/:job_id", h.GetConvertLocalGIFZipToWebPJob)
			...
			admin.GET("/upload-rules", h.GetAdminUploadRules)
			admin.PUT("/upload-rules", h.UpdateAdminUploadRules)
			admin.GET("/ugc/reviews", h.ListAdminUGCReviews)
			admin.GET("/ugc/reviews/:id", h.GetAdminUGCReviewDetail)
			admin.GET("/ugc/reviews/:id/logs", h.ListAdminUGCReviewLogs)
			admin.POST("/ugc/reviews/:id/approve", h.AdminApproveUGCReview)
			admin.POST("/ugc/reviews/:id/reject", h.AdminRejectUGCReview)
			admin.POST("/ugc/reviews/:id/offline", h.AdminOfflineUGCReview)
			admin.POST("/ugc/reviews/batch/approve", h.AdminBatchApproveUGCReview)
			admin.POST("/ugc/reviews/batch/reject", h.AdminBatchRejectUGCReview)
			admin.POST("/ugc/reviews/batch/offline", h.AdminBatchOfflineUGCReview)
			...
			admin.GET("/video-jobs/ingress-jobs", h.ListAdminVideoIngressJobs)
			...
			admin.GET("/video-jobs/ai-prompt-templates/fixed", h.ListAdminVideoAIPromptFixedTemplates)
			admin.GET("/video-jobs/ai-prompt-templates/fixed/:id", h.GetAdminVideoAIPromptFixedTemplate)
			admin.POST("/video-jobs/ai-prompt-templates/fixed", h.CreateAdminVideoAIPromptFixedTemplate)
			admin.PUT("/video-jobs/ai-prompt-templates/fixed/:id", h.UpdateAdminVideoAIPromptFixedTemplate)
			admin.DELETE("/video-jobs/ai-prompt-templates/fixed/:id", h.DeleteAdminVideoAIPromptFixedTemplate)
